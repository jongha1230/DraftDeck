...
DraftDeck은 범용 AI 작성기가 아니라 기술 글 초안을 빠르게 잡기 위한 개인용 글쓰기 도구입니다.
핵심은 자료를 가져와 초안으로 구조화하고, 선택한 문장만 AI로 다듬은 뒤 자동 저장, 버전 기록, 마크다운 내보내기까지 한 흐름으로 이어주는 것입니다.
...
- 자동 저장과 버전 기록: [docs/autosave-and-revisions.md](./docs/autosave-and-revisions.md)
...
- Problem:
  - 자료 가져오기, 초안 작성, 선택 기반 AI 편집, 자동 저장, 버전 추적이 따로 놀면 글쓰기 흐름을 설명하기 어렵다.
- Key features:
  - 자료 가져오기, 자료 기반 초안 생성, 선택 기반 AI 편집, 자동 저장과 버전 기록, 마크다운 내보내기
- Tech stack:
  - Next.js 16, React 19, TypeScript, Zustand, Supabase, Gemini API, Vitest, Playwright
- Architecture highlights:
  - 서버 액션 경계, Supabase 저장 계층, AI 서비스 분리, 클라이언트 세션 저장, 데모 모드
- Testing & quality:
  - Vitest 단위 테스트, Playwright 데모 흐름, 실제 AI 호출 없는 테스트, 버전 충돌 테스트, 자료 파일 제한 검증
- Known limitations:
...
협업, 여러 기기 동기화, 분석 기능, PDF 내보내기는 이번 범위에서 제외했습니다.
...
- `draft_revisions`
  - 자동 저장, AI 결과 적용, 자료 가져오기를 버전 기록으로 남김
...
- `src/lib/drafts/persistence.ts`
  - 문서 생성/조회/수정/삭제, 자료 기록, 버전 기록, 작업 결과 조회
...
- `src/hooks/useDraftPageController.ts`
  - 자동 저장 대기열, 데모 세션 복원, 자료 가져오기/AI 실행/결과 적용 흐름

### 3. Demo mode

- auth를 끄지 않고 UI만 빠르게 검증하기 위한 bypass
- localStorage에 데모 세션을 저장해 새로고침 후에도 흐름 유지
- local:
...
- 자료 가져오기, AI 결과 적용, 버전 기록을 로그인 없이 한 흐름으로 체험할 수 있습니다.
- guest demo 세션은 브라우저 localStorage에만 저장됩니다.
- demo 안의 AI 결과는 실제 모델 호출이 아니라 로컬 예시 결과입니다.
...
### 2. 자동 저장과 사용자용 최근 버전이 섞이던 문제

한 글자만 바뀌어도 자동 저장은 돌아야 합니다. 문제는 그 저장을 그대로 `최근 버전 vN`처럼 보여주면, 사용자가 실제로 되돌릴 가치가 있는 체크포인트와 단순 저장 상태를 구분할 수 없다는 점이었습니다.

그래서 `자동 저장`과 `최근 버전`을 분리했습니다. 자동 저장은 계속 DB를 최신 상태로 밀어주지만, 사용자에게 보이는 최근 버전은 `의미 있는 수정`이나 `AI 결과를 실제 적용한 경우`에만 생성되도록 체크포인트 기준을 분리했습니다. 이 분리 덕분에 저장 상태는 즉시 알리면서도, 버전 기록은 되돌릴 가치가 있는 기록만 남길 수 있게 됐습니다.
